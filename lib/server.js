    ...
    start() {
      ...
      return Promise.all([
        ...
        .then(([port, hotPort]) => {
          options.port = port;
          if (options.hot) {
            // if the hot port option passed was a number, use the getPort result
            // otherwise, it should be an object structured as {client: <browser => server port>, server: <websocket listen port>}
            if (typeof options.hot.port === 'number') {
